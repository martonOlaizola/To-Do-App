...
def create_task(task: TaskCreate, db: Session = Depends(get_db), current_user: dict = Depends(auth.get_current_user)):
  ...
  task_db = task_service.create_task(task, user_id, db)
  return task_db
...
def get_all_task(db: Session = Depends(get_db)):
  ...
  """
  task_db = task_service.get_all_tasks(db)
  return task_db
...
def get_task_by_id(task_id: int, db: Session = Depends(get_db)):
  ...
  """
  task_db = task_service.get_task_by_id(task_id, db)
  if task_db is None:
    raise HTTPException(
      status_code=400,
...
def get_all_tasks_from_user(user: dict = Depends(auth.get_current_user), db: Session = Depends(get_db)):
  ...
  task_db = task_service.get_all_tasks_from_user(user_id, db)
  return task_db
...
def update_task(task: TaskUpdate, task_id: int, user: dict = Depends(auth.get_current_user), db: Session = Depends(get_db)):
  ...
  return task_service.update_task(task, task_id, user_id, db)
...
def delete_task(task_id: int, user: dict = Depends(auth.get_current_user), db: Session = Depends(get_db)):
  ...
  """
  user_id = int(user['user_id'])
  task_db = task_service.delete_task(task_id, user_id, db)
  return task_db
...
def delete_completed_tasks(user: dict = Depends(auth.get_current_user), db: Session = Depends(get_db)):
  ...
  """
  user_id = int(user['user_id'])
  task_db = task_service.delete_completed_tasks(user_id, db)
  return task_db
